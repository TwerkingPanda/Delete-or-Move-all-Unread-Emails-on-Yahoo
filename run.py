...
#m.fetch(messageID, '(RFC822)')				#retrieves the message from messageID. To view the message.
...
for em_id in unread_count:			
	...
	print('Delete Successful for emailID: {em_id}')
'''
...
for j in range(0,len(unread_count),500):
	...
	m.expunge()
	print('Delete Successful for emailIDs: {','.join(str(i)[2:-1] for i in unread_count[j:j+500])}')
m.expunge()
m.logout()
